function generateTemplate(data, role) {
  ...
  if (role === 'parent') {
    const { childName, childAge, grade, subject } = data;
    return [`שלום, אני הורה לילד/ה בשם ${childName}, בגיל ${childAge}, הלומד/ת בכיתה ${grade}. ברצוני לסייע לו/לה להבין טוב יותר את מקצוע ה-${subject} בו הוא/היא חווה קושי. אשמח לקבל הסבר מפורט על נושאים מרכזיים במקצוע זה ברמת הכיתה, כולל דוגמאות פשוטות, המחשות, ותרגול מותאם. כמו כן, אודה להמלצה על אסטרטגיות למידה מתאימות לפי הגיל ורמת ההבנה.`,

    `שלום! אני מחפש/ת עזרה לימודית עבור ${childName}, בן/בת ${childAge}, תלמיד/ה בכיתה ${grade}. נושא הלימוד שבו אנו נתקלים בקושי הוא ${subject}. אני זקוק/ה לתוכן לימודי מותאם אישית שיכלול:
    ...
    תודה רבה מראש!`,
      ...
      `שלום מערכת בינה מלאכותית, אנא עזרי לי ליצור תוכנית תמיכה לימודית ב-${subject} עבור ילדי ${childName}, בן/בת ${childAge}, בכיתה ${grade}. 
